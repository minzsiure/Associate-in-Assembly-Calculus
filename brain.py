...
class Brain:
    ...
    def add_stimulus(self, name, k):
        ...
        for key in self.areas:
            if self.areas[key].explicit:
                new_connectomes[key] = np.random.binomial(
                    k, self.p, size=(self.areas[key].n)) * 1.0
            else:
    ...
    def add_explicit_area(self, name, n, k, beta):
        ...
        for stim_name, stim_connectomes in self.stimuli_connectomes.items():
            stim_connectomes[name] = np.random.binomial(
                self.stimuli[stim_name].k, self.p, size=(n)) * 1.0
            self.areas[name].stimulus_beta[stim_name] = beta
        ...
        for key in self.areas:
            if key == name:  # create explicitly
                new_connectomes[key] = np.random.binomial(
                    1, self.p, size=(n, n)) * 1.0
            if key != name:
                if self.areas[key].explicit:
                    other_n = self.areas[key].n
                    new_connectomes[key] = np.random.binomial(
                        1, self.p, size=(n, other_n)) * 1.0
                    self.connectomes[key][name] = np.random.binomial(
                        1, self.p, size=(other_n, n)) * 1.0
    ...
    def project(self, stim_to_area, area_to_area, verbose=False):
        ...
        for area in to_update:
            num_first_winners = self.project_into(
                self.areas[area], stim_in[area], area_in[area], verbose)
            self.areas[area].num_first_winners = num_first_winners
            if self.save_winners:
                self.areas[area].saved_winners.append(
                    self.areas[area].new_winners)
    ...
    def project_into(self, area, from_stimuli, from_areas, verbose=False):
        ...
        print("\tProjecting " + ",".join(from_stimuli) + " and " +
              ",".join(from_areas) + " into " + area.name)
        ...
        for from_area in from_areas:
            if not self.areas[from_area].winners or (self.areas[from_area].w == 0):
                raise Exception(
                    "Projecting from area with no assembly: " + from_area)
        ...
        if not area.explicit:
            ...
            if verbose:
                print("total_k = " + str(total_k) +
                      " and input_sizes = " + str(input_sizes))
            ...
            alpha = binom.ppf((float(effective_n-area.k) /
                              effective_n), total_k, self.p)
            if verbose:
        ...
        new_winner_indices = heapq.nlargest(area.k, range(
            len(all_potential_winners)), all_potential_winners.__getitem__)
        num_first_winners = 0
            ...
            for i in range(area.k):
                if new_winner_indices[i] >= area.w:
                    first_winner_inputs.append(
                        potential_new_winners[new_winner_indices[i] - area.w])
        ...
        for i in range(num_first_winners):
            input_indices = random.sample(
                range(0, total_k), int(first_winner_inputs[i]))
            inputs = np.zeros(num_inputs)
            total_so_far = 0
            for j in range(num_inputs):
                inputs[j] = sum(
                    [((total_so_far + input_sizes[j]) > w >= total_so_far) for w in input_indices])
                total_so_far += input_sizes[j]
            first_winner_to_inputs[i] = inputs
            if verbose:
                print("for first_winner # " + str(i) + " with input " +
                      str(first_winner_inputs[i]) + " split as so: ")
                print(inputs)
        ...
        for stim in from_stimuli:
            ...
            for i in range(num_first_winners):
                self.stimuli_connectomes[stim][name][area.w +
                                                     i] = first_winner_to_inputs[i][m]
        ...
        for from_area in from_areas:
            ...
            for i in range(num_first_winners):
                ...
                sample_indices = random.sample(
                    from_area_winners, int(total_in))
                for j in range(from_area_w):
                    ...
                    if j not in from_area_winners:
                        self.connectomes[from_area][name][j][area.w +
            ...
            for i in area.new_winners:
                for j in from_area_winners:
                    self.connectomes[from_area][name][j][i] *= (
                        1.0 + area_to_area_beta)
            if verbose:
                print("Connectome of " + from_area +
                      " to " + name + " is now:")
        ...
        for other_area in self.areas:
            if other_area not in from_areas:
                ...
                for j in range(self.areas[other_area].w):
                    for i in range(area.w, area.new_w):
                        self.connectomes[other_area][name][j][i] = np.random.binomial(
                            1, self.p)
            ...
            for i in range(area.w, area.new_w):
                for j in range(columns):
                    self.connectomes[name][other_area][i][j] = np.random.binomial(
                        1, self.p)
            if verbose:
                print("Connectome of " + name +
                      " to " + other_area + " is now:")
...
if __name__ == '__main__':
    b = Brain(p=0.01)
    print('brain areas', b.areas)
    b.add_stimulus("stimA", k=4)
    print()
    print('stimuli_connectomes', b.stimuli_connectomes)
    print('brain areas', b.areas)
    print('connectomes', b.connectomes)
